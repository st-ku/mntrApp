...
import static main.java.com.app.mntr.extension.WebUtils.getRequestParam;
import static main.java.com.app.mntr.extension.WebUtils.getValues;
    ...
    void execute(final HttpExchange httpExchange) throws IOException {
        ...
        if (GET.equals(method)) {
            final OperationResponse<Collection<Config>> response = getRequestParam(uri, REQ_PARAM_NAMES).
                    map(param -> {
                        try {
                            ...
                            return new OperationResponse.Builder<Collection<Config>>().error(Constants.Messages.STRING_TO_JSON_ERROR).build();
                        }
                    }).
                    orElseGet(() -> {
            ...
            try (final BufferedReader bufferedReader =
                ...
                LOGGER.log(Level.SEVERE, e.toString());
                throw new InvalidRequestException(HTTP_BAD_REQUEST, Constants.Messages.JSON_TO_CONFIG_ERROR);
            }
            ...
            final OperationResponse<Integer> response = getRequestParam(uri, REQ_PARAM_NAMES).
                    map(param -> {
                        try {
                            ...
                            LOGGER.log(Level.SEVERE, e.toString());
                            return new OperationResponse.Builder<Integer>().error(Constants.Messages.STRING_TO_JSON_ERROR).build();
                        }
                    }).
                    orElseGet(() -> new OperationResponse.Builder<Integer>().error(Constants.Messages.REQUEST_PARAM_NOT_PRESENT).build());
            writeResponse(httpExchange, response);
        } else {
